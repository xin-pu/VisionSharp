...
{
    public abstract class General : AbstractTest
    {
        ...
        public TrayLayoutPredictor Predictor { set; get; }
        ...
        public abstract TrayLayoutPredictor InitialPreidctor();
        ...
        public void GeneraitAnnToImage(string workfolder)
        {
            ...
            new DirectoryInfo(annFolder).GetFiles("*.txt")
                ...
                .ForEach(ann =>
                {
                    ...
                    var detectLayer = DetectLayout.LoadFromAnnotation(ann.FullName);
                    var mat = Cv2.ImRead(img);
                    new TrayLayoutPainter().Call(new Tuple<Mat, DetectLayout>(mat, detectLayer), mat, id);
                    mat.Dispose();
                    GC.Collect();
        ...
        internal void predictFolder(string imageFolder)
        {
            ...
            images.ForEach(img =>
            {
                ...
                DetectLayout.SaveAnnotation(annfile, layoutPredict);
            });
        }
        ...
        internal void filterNotMatch(string workfolder, string outfolder)
        {
            ...
            allann.ForEach(ann =>
            {
                ...
                var layoutTarget = DetectLayout.LoadFromAnnotation(annfile);
                var layoutPredict = predictSingle(new FileInfo(imgFile));
                ...
                if (!isEqual)
                {
                    ...
                    DetectLayout.SaveAnnotation(outAnnFile, layoutPredict);
                }
            });
